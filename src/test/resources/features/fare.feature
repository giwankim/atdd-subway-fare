  ...
  */
  ...
  Rule: 사용자의 연령별 요금 할인
    ...
    Scenario: 청소년 운임은 350원을 공제한 금액의 20% 할인
      Given "youth@example.com"과 "secret"으로 로그인을 하고2
      When "교대역"에서 "양재역"까지 최소 시간 경로를 조회하면2
      Then 총 거리는 50km이며 총 소요 시간은 5분이다2
      And 이용 요금은 1710원이다2

    Scenario: 어린이 운임은 350원을 공제한 금액의 50% 할인
      Given "child@example.com"과 "secret"으로 로그인을 하고2
      When "교대역"에서 "양재역"까지 최소 시간 경로를 조회하면2
      Then 총 거리는 50km이며 총 소요 시간은 5분이다2
      And 이용 요금은 1200원이다2
  ...
  Rule: 노선별 추가 요금을 부가
    ...
    Scenario: 추가 요금이 있는 노선을 이용 할 경우 측정된 요금에 추가
      Given "adult@example.com"과 "secret"으로 로그인을 하고2
      When "교대역"에서 "양재역"까지 최소 시간 경로를 조회하면2
      Then "교대역,강남역,양재역" 경로가 조회된다2
      And 이용 요금은 2950원이다2

    Scenario: 경로 중 추가요금이 있는 노선을 환승 하여 이용 할 경우 가장 높은 금액의 추가 요금만 적용
      Given "adult@example.com"과 "secret"으로 로그인을 하고2
      When "교대역"에서 "양재역"까지 최단 거리 경로를 조회하면2
      Then "교대역,남부터미널역,양재역" 경로가 조회된다2
      And 이용 요금은 2350원이다2
