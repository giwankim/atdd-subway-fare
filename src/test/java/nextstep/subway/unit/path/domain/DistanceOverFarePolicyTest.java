...
class DistanceOverFarePolicyTest {
  ...
  @ParameterizedTest
  @MethodSource
  void calculateOverFare(long distance, long expectedFare) {
    List<Station> stations = Arrays.asList(교대역(), 강남역());
    OverFarePolicy policy = new DistanceOverFarePolicy(10, 50, 100, 5);
    assertThat(policy.calculateOverFare(Path.of(stations, distance, 10))).isEqualTo(expectedFare);
  }
